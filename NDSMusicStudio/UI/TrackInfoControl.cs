    ...
    class TrackInfoControl : UserControl
    {
        ...
        void TogglePiano(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                pianos[16].CheckState = on == 0x10 ? CheckState.Checked : (on == 0 ? CheckState.Unchecked : CheckState.Indeterminate);
                pianos[16].CheckStateChanged += TogglePiano;
            }
        }
        void ToggleMute(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                mutes[16].CheckState = on == 0x10 ? CheckState.Checked : (on == 0 ? CheckState.Unchecked : CheckState.Indeterminate);
                mutes[16].CheckStateChanged += ToggleMute;
            }
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            for (int i = 0; i < 0x10; i++)
            {
                ...
                e.Graphics.DrawString(Info.Portamentos[i].ToString(), Font, Brushes.HotPink, voicesX + (row2ElementAdditionX * 5), r2y);
                ...
                string theseNotes = string.Join(" ", Info.Notes[i].Select(n => Utils.NoteNames[n % 12] + ((n / 12) - 2)));
